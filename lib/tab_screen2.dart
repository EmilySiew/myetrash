...
import 'package:myetrash/newetrash.dart';
...
class _TabScreen2State extends State<TabScreen2> {
  ...
  void requestNewETrash() {
    print(widget.user.email);
    if (widget.user.email != "user@noregister") {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (BuildContext context) => NewETrash(
                    user: widget.user,
                  )));
    } else {
      Toast.show("Please Register First to request new etrash", context,
          duration: Toast.LENGTH_LONG, gravity: Toast.BOTTOM);
      Navigator.push(
          context,
          MaterialPageRoute(
  ...
  void _onEtDelete(String etid, String etname) {
    print("Delete " + etid);
  ...
  Future<String> deleteRequest(String etid) async {
    String urlLoadETrash = "http://itschizo.com/emily_siew/myETrash/php/delete_etrash.php";
    ProgressDialog pr = new ProgressDialog(context,
        type: ProgressDialogType.Normal, isDismissible: false);
    pr.style(message: "Deleting ETrash");
    pr.show();
    http.post(urlLoadETrash, body: {
      "etid": etid,
    }).then((res) {
